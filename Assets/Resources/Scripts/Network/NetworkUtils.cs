...
public static class NetworkUtils
{
    ...
    {
        return Quaternion.Inverse(me) * other;
    }

    public static Quaternion AverageRotations(this IEnumerable<Quaternion> quats)
    {
        if (!quats.Any())
            return Quaternion.identity;

        Vector4 output = Vector4.zero;
        Quaternion first = quats.First();

        var i = 1;

        foreach (var quat in quats)
            AverageQuaternion(ref output, quat, first, i++);
